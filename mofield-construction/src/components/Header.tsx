...
const Header = () => {
  ...
  return (
    <>
      <nav
        className={`navbar justify-between text-white max-w-[2500px] fixed top-0 z-30 px-6 md:px-24 py-2 transition duration-300 ${
          isScrolled ? "backdrop-blur-sm bg-yellow-950 bg-opacity-70 dark:bg-gray-700 dark:bg-opacity-70" : "bg-transparent"
        }`}
      >
        <div className="">
          ...
          {isOpen && (
            ...
            >
              <ul tabIndex={0} className="text-left flex flex-col text-xl w-full pl-10 py-2 font-semibold">
                <li className={`py-2 ${bitter.className}`} onClick={toggleMenu}>
                  <Link href="/">Home</Link>
                </li>
                <li className={`py-2 ${bitter.className}`} onClick={toggleMenu}>
